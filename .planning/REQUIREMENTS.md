...
- [x] **REP-01**: MIT LICENSE 파일이 레포 루트에 존재한다
- [x] **REP-02**: `.planning/` 디렉토리가 git 추적에서 제거된다 (로컬은 유지)
- [x] **REP-03**: .gitignore에 이미 선언되었으나 git에 남아있는 스테일 파일이 정리된다 (tests/, scripts/)
- [x] **REP-04**: .gitignore가 개발 산출물, 에디터 파일, AI 계획 파일을 완전히 커버한다

### Package Quality

- [x] **PKG-01**: package.json에 `repository`, `homepage`, `bugs` 필드가 존재한다
- [x] **PKG-02**: `engines` 필드가 `>=20.11.0`으로 설정된다 (`import.meta.dirname` 최소 요구)
- [x] **PKG-03**: `files` 필드가 런타임 필수 파일만 포함한다 (dev scripts, 중복 hooks, .d.ts 제외)
- [x] **PKG-04**: `prepublishOnly`가 build와 test를 모두 실행한다
- [x] **PKG-05**: `id3-start/SKILL.md`에서 `npx id3-cli .`가 `id3-cli init .`로 변경된다

### Documentation

- [x] **DOC-01**: README.md에서 Quick Start(설치+첫 사용)가 상단 15줄 이내에 위치한다
- [ ] **DOC-02**: 모든 README(6개)에서 npx 참조가 글로벌 설치 안내로 교체된다
- [x] **DOC-03**: CHANGELOG.md가 Keep a Changelog 형식으로 존재한다
- [ ] **DOC-04**: 번역 README(4개)가 새로운 구조에 맞게 업데이트된다
- [x] **DOC-05**: README에 npm version, license 배지가 추가된다
