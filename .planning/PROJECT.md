...
- [ ] npm 글로벌 설치 방식으로 전환 (`npm i -g id3-cli`)
- [ ] 글로벌 설치 후 `/id3-init` 슬래시 커맨드로 프로젝트 셋업
- [ ] `/id3-start` 스마트 라우터 스킬 추가
- [ ] `/id3-start [요청]` 입력 시 요청 내용 분석 후 적절한 Phase로 자동 라우팅
- [ ] 라우팅 시 사용자에게 어떤 단계로 진행하는지 가이드 메시지 제공
- [ ] 정보정의가 불필요한 UI 요청은 id3-design-ui로 직행
...
|----------|-----------|---------|
| npm 글로벌 설치 방식 채택 | 슬래시 커맨드 등록을 위해 글로벌 패키지가 필요 | Pending |
| /id3-start를 별도 스킬로 추가 | 기존 스킬 로직은 변경하지 않고 라우터 역할만 담당 | Pending |
| 기존 npx 방식 하위 호환 유지 | 기존 사용자 영향 최소화 | Pending |
