...
グローバルインストールにより、すべてのプロジェクトで使用できる2つのグローバルスキル（`/id3-start` と `/id3-clear`）が登録されます。その後、任意のプロジェクトで `/id3-start` を実行するだけです -- IDDDが設定済みかどうかを自動検出し、すべてを処理します。
...
`npm i -g id3-cli` を実行した後、以下のグローバルスキルがシステム全体にインストールされます：

```
~/.claude/skills-global/              グローバルスキル（npm i -g でインストール）
  ├── id3-start/                      スマートルーターエントリーポイント
  │   ├── SKILL.md
  │   └── references/
  │       ├── phase-guide.md            Phase ルーティング分類体系
  │       └── dashboard-template.md     進行ダッシュボード形式
  └── id3-clear/                      プロジェクトリセット
      └── SKILL.md
```

`npx id3-cli@latest` を実行した後（または `/id3-start` によって自動的に）、プロジェクトに以下の構造が追加されます：
...
> **ヒント：** 個別の Phase コマンドを覚える必要はありません。`/id3-start [リクエスト]` を使用してください。進行ダッシュボードを表示し、意図を分析した後、正しい Phase スキルへ自動的にルーティングします。これが推奨されるエントリーポイントです。

**Phase 0/1 -- Entity 識別：**
AIコーディングエージェントを開き、`/id3-start ドメインの Entity を識別して` を実行します（または直接 `/id3-identify-entities`）。エージェントは既存のコードベースがあるか（ブラウンフィールド）、新規プロジェクトか（グリーンフィールド）を自動検出し、適切な識別フローを実行します。

**Phase 2 -- 情報設計：**
`/id3-start 情報モデルを洗練して` を実行します（または直接 `/id3-design-information`）。エージェントは概念モデルを論理モデルに洗練し、ビジネスルールを導出し、バージョンヘッダーと Hook 設定をセットアップします。

**Phase 2.5 -- UI設計：**
`/id3-start UI を設計して` を実行します（または直接 `/id3-design-ui`）。エージェントは Entity カタログから画面構造を導出し、デザイントークンでビジュアルデザインコントラクトを確立し、インタラクティブなモックアッププレビュー付きの 7-Pillar 品質ゲートを実行した後、Agent Teams を生成して画面を並列実装し、事後監査を行います。

**Phase 3-5 -- Agent Teams による実装：**
`/id3-start システムをビルドして` を実行します（または直接 `/id3-spawn-team`）。エージェントは確定した情報モデルを読み込み、専門化されたエージェントチーム（spec-generator、implementer、qa-reviewer）を生成して、並列でシステムを実装します。

---

## スキル

### id3-start (グローバル -- スマートルーター)

IDDDのスマートエントリーポイントです。ユーザーが個別の Phase スキル名を覚える必要はありません -- `/id3-start` がリクエストを分析して正しいスキルへ自動的にルーティングします。

**トリガーキーワード：** `start IDDD`、`begin project`、`what should I do next`、`identify entities`、`design information`、`design ui`、`build`、`audit`、`preview`

**機能：**

1. **自動セットアップ：** 現在のプロジェクトに IDDD がインストールされているかを検出します（`specs/entity-catalog.md` + `CLAUDE.md`）。インストールされていない場合、自動的に `npx id3-cli .` を実行して IDDD をセットアップしてから続行します。
2. **進行ダッシュボード：** 各 Phase（Phase 0/1、Phase 2、Phase 2.5、Phase 3-5）の完了状態をビジュアルシンボル（完了チェックマーク、進行中ダイヤモンド、未開始サークル）と進行バーで表示する Phase パイプラインを表示します。
3. **意図ルーティング：** ユーザーの自然言語リクエストを Phase シグナルキーワードと照合・分析し、正しい Phase スキル（`/id3-identify-entities`、`/id3-design-information`、`/id3-design-ui`、`/id3-spawn-team`、`/id3-info-audit`、または `/id3-preview`）へルーティングします。
4. **曖昧なリクエストの処理：** リクエストが複数の Phase にマッチする可能性がある場合（例：「リストにフィルターを追加」-- UI のみの変更か、新しいデータ Entity が必要かが不明な場合）、ルーティング前に明確化の質問をします。
5. **UI ファストパス：** リクエストが明示的な UI キーワードのみを含み、データモデルがすでに存在する場合（version >= 1.0）、Entity について確認せずに直接 `/id3-design-ui` へルーティングします。
6. **前提条件チェック：** 対象 Phase の前提条件が満たされていない場合、警告を表示して正しい開始 Phase を提案します。

**使用法：**

```
/id3-start                          ダッシュボードを表示 + 次のアクションを提案
/id3-start identify the entities    /id3-identify-entities へルーティング
/id3-start design the UI            /id3-design-ui へルーティング
/id3-start build the system         /id3-spawn-team へルーティング
/id3-start run an audit             /id3-info-audit へルーティング
```

**インストール：** `npm i -g id3-cli` でグローバルインストール、`~/.claude/skills-global/id3-start/` に配置されます。
...
### id3-clear (グローバル -- プロジェクトリセット)

現在のプロジェクトから IDDD が生成したすべてのファイルを安全に削除し、IDDD 導入前の状態に復元します。

**トリガーキーワード：** `clear iddd`、`reset iddd`、`remove iddd`、`clean project`

**手順：**

1. **インストール確認：** プロジェクトに IDDD ファイルが存在するか確認します。見つからない場合は「IDDD ファイルが見つかりません」と報告して停止します。
2. **削除対象のスキャン：** どの IDDD ディレクトリ（`specs/`、`docs/`、`steering/`、`hooks/`、`skills/`、`.claude/skills/`、`.claude/hooks/`、`.codex/skills/`、`.agents/skills/`、`.iddd/`）とファイル（`CLAUDE.md`、`AGENTS.md`）が実際に存在するかを識別します。
3. **警告の表示：** 削除されるすべてのファイルとディレクトリの詳細リストを表示します。ユーザー作成ファイル（`steering/product.md`、`steering/data-conventions.md`）には特別な注釈を追加します。
4. **確認の要求：** `[y/N]`（デフォルト N）でプロンプトを表示します。明示的に「y」または「yes」と入力した場合のみ続行します。
5. **削除の実行：** 識別された対象のみを削除します。削除件数を含む完了サマリーを表示します。

**安全ルール：**
- 既知の IDDD ファイルリスト以外のファイルは絶対に削除しない
- `rm -rf *` のようなグロブパターンは絶対に使用しない
- 確認ステップは絶対にスキップしない
- 選択的な削除が必要な場合は、手動ファイル操作を使用してください

**インストール：** `npm i -g id3-cli` でグローバルインストール、`~/.claude/skills-global/id3-clear/` に配置されます。
...
> /id3-start モデルを洗練して
  ...
  Routing to /id3-design-information -- 概念モデルを論理モデルに洗練します。
...
> /id3-start UI を設計して
  ...
  Routing to /id3-design-ui -- 情報モデルから導出した UI を設計・実装します。
...
> /id3-start ビルドして
  ...
  Routing to /id3-spawn-team -- 並列実装のために Agent Teams を生成します。
