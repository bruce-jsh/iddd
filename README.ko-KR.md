...
프로젝트에서 `/id3-start`를 실행하세요. IDDD 설치 상태를 자동 감지하고, 진행 대시보드와 함께 다음에 실행할 명령어를 안내합니다.
...
글로벌 설치 시 두 개의 글로벌 스킬(`/id3-start`, `/id3-clear`)이 등록되어 모든 프로젝트에서 사용할 수 있습니다. 프로젝트에서 처음 `/id3-start`를 실행하면 IDDD 설치 여부를 자동 감지하고 다음 단계를 안내합니다.
...
> **팁:** 처음에 `/id3-start`를 실행하면 현재 진행 상태와 다음 단계를 안내합니다. 이후에는 아래 개별 명령어를 직접 사용하세요.

**Phase 0/1: Entity 식별:**
`/id3-identify-entities`를 실행하세요. 에이전트가 기존 코드베이스가 있는지(브라운필드) 새로 시작하는지(그린필드)를 자동으로 감지한 후, 적절한 식별 플로우를 실행합니다.
...
IDDD의 진입점입니다. 처음 한 번 실행하면 프로젝트 설정, 진행 대시보드, 다음 단계 안내를 제공합니다. 요청을 함께 전달하면 올바른 Phase 스킬로 라우팅합니다. 이후에는 안내받은 개별 명령어를 직접 사용하세요.
...
3. **의도 라우팅:** 요청을 함께 전달하면 올바른 Phase 스킬(`/id3-identify-entities`, `/id3-design-information`, `/id3-design-ui`, `/id3-spawn-team`, `/id3-info-audit`, 또는 `/id3-preview`)로 라우팅합니다.
