...
`npm i -g id3-cli` 실행 후, 다음 전역 스킬이 시스템 전체에 설치됩니다:

```
~/.claude/skills-global/              전역 스킬 (npm i -g로 설치)
  ├── id3-start/                      스마트 라우터 진입점
  │   ├── SKILL.md
  │   └── references/
  │       ├── phase-guide.md            Phase 라우팅 분류 체계
  │       └── dashboard-template.md     진행 대시보드 형식
  └── id3-clear/                      프로젝트 초기화
      └── SKILL.md
```
...
> **팁:** 개별 Phase 명령어를 기억할 필요 없이, `/id3-start [요청]`을 사용하세요. 진행 대시보드를 표시하고, 의도를 분석한 후, 올바른 Phase 스킬로 자동 라우팅합니다. 이것이 권장 진입점입니다.

**Phase 0/1 -- Entity 식별:**
AI 코딩 에이전트를 열고 `/id3-start 도메인의 Entity를 식별해줘` (또는 직접 `/id3-identify-entities`)를 실행하세요. 에이전트가 기존 코드베이스가 있는지(브라운필드) 새로 시작하는지(그린필드)를 자동으로 감지한 후, 적절한 식별 플로우를 실행합니다.

**Phase 2 -- 정보 설계:**
`/id3-start 정보 모델을 정제해줘` (또는 직접 `/id3-design-information`)를 실행하세요. 에이전트가 개념 모델을 논리 모델로 정제하고, 비즈니스 규칙을 도출하며, 버전 헤더와 Hook 설정을 구성합니다.

**Phase 2.5 -- UI 설계:**
`/id3-start UI를 설계해줘` (또는 직접 `/id3-design-ui`)를 실행하세요. 에이전트가 Entity 카탈로그에서 화면 구조를 도출하고, 디자인 토큰으로 시각적 디자인 계약을 수립하며, 인터랙티브 목업 프리뷰와 함께 7-Pillar 품질 게이트를 실행한 후, Agent Teams를 생성하여 화면을 병렬 구현하고 사후 감사를 수행합니다.

**Phase 3-5 -- Agent Teams를 통한 구현:**
`/id3-start 시스템을 빌드해줘` (또는 직접 `/id3-spawn-team`)를 실행하세요. 에이전트가 확정된 정보 모델을 읽고 전문 에이전트 팀(spec-generator, implementer, qa-reviewer)을 생성하여 시스템을 병렬로 구현합니다.

---

## 스킬

### id3-start (전역 -- 스마트 라우터)

IDDD의 스마트 진입점입니다. 사용자가 개별 Phase 스킬 이름을 기억할 필요 없이 -- `/id3-start`가 요청을 분석하여 올바른 스킬로 자동 라우팅합니다.

**트리거 키워드:** `start IDDD`, `begin project`, `what should I do next`, `identify entities`, `design information`, `design ui`, `build`, `audit`, `preview`
...
5. **UI 빠른 경로:** 요청이 명시적 UI 키워드만 포함하고 데이터 모델이 이미 존재하는 경우(version >= 1.0), Entity 질문 없이 직접 `/id3-design-ui`로 라우팅합니다.
6. **전제 조건 확인:** 대상 Phase의 전제 조건이 충족되지 않으면 경고하고 올바른 시작 Phase를 제안합니다.

**사용법:**

```
/id3-start                             대시보드 표시 + 다음 작업 제안
/id3-start Entity를 식별해줘           /id3-identify-entities로 라우팅
/id3-start UI를 설계해줘               /id3-design-ui로 라우팅
/id3-start 시스템을 빌드해줘           /id3-spawn-team으로 라우팅
/id3-start 감사를 실행해줘             /id3-info-audit로 라우팅
```

**설치:** `npm i -g id3-cli`로 `~/.claude/skills-global/id3-start/`에 전역 설치됩니다.
...
### id3-clear (전역 -- 프로젝트 초기화)

현재 프로젝트에서 IDDD가 생성한 모든 파일을 안전하게 제거하여, IDDD 설치 이전 상태로 복원합니다.

**트리거 키워드:** `clear iddd`, `reset iddd`, `remove iddd`, `clean project`

**절차:**

1. **설치 확인:** 프로젝트에 IDDD 파일이 존재하는지 확인합니다. 없으면 "IDDD 파일을 찾을 수 없습니다"라고 보고하고 중단합니다.
2. **삭제 대상 스캔:** 어떤 IDDD 디렉터리(`specs/`, `docs/`, `steering/`, `hooks/`, `skills/`, `.claude/skills/`, `.claude/hooks/`, `.codex/skills/`, `.agents/skills/`, `.iddd/`)와 파일(`CLAUDE.md`, `AGENTS.md`)이 실제로 존재하는지 식별합니다.
3. **경고 표시:** 삭제될 모든 파일과 디렉터리의 상세 목록을 표시합니다. 사용자 작성 파일(`steering/product.md`, `steering/data-conventions.md`)에 대해서는 특별 주석을 추가합니다.
4. **확인 요구:** `[y/N]`으로 프롬프트합니다 (기본값 N). 명시적으로 "y" 또는 "yes"를 입력한 경우에만 진행합니다.
5. **삭제 실행:** 식별된 대상만 제거합니다. 삭제 수량이 포함된 완료 요약을 표시합니다.

**안전 규칙:**
- 알려진 IDDD 파일 목록 이외의 파일은 절대 삭제하지 않음
- `rm -rf *` 같은 글로브 패턴을 절대 사용하지 않음
- 확인 단계를 절대 건너뛰지 않음
- 선택적 삭제가 필요하면 수동 파일 작업을 사용하세요

**설치:** `npm i -g id3-cli`로 `~/.claude/skills-global/id3-clear/`에 전역 설치됩니다.
...
> /id3-start
  ...
  (대시보드에 모든 Phase가 ○ -- 미시작으로 표시)

  > Suggested next action: Entity Identification (Phase 0/1).
  > Use `/id3-start [your request]` to begin.

> /id3-start SaaS 도메인의 Entity를 식별해줘

  Routing to /id3-identify-entities -- 구조화 인터뷰를 통해 도메인 Entity를 식별합니다.
  이 Phase에서 생성되는 산출물: specs/entity-catalog.md, specs/data-model.md, docs/business-rules.md

  Agent: "시스템이 관리하는 핵심 '사물'은 무엇입니까?"
  You: "Users, Organizations, Subscriptions, Invoices, Features"
  Agent: "Users와 Organizations는 어떻게 관련됩니까?"
  You: "role Attribute를 가진 Membership Entity를 통한 다대다 관계입니다."
  ...
...
> /id3-start 모델을 정제해줘
  ...
  Routing to /id3-design-information -- 개념 모델을 논리 모델로 정제합니다.
...
> /id3-start UI를 설계해줘
  ...
  Routing to /id3-design-ui -- 정보 모델에서 UI를 도출하고 구현합니다.
...
> /id3-start 빌드
  ...
  Routing to /id3-spawn-team -- Agent Teams를 생성하여 병렬 구현합니다.
